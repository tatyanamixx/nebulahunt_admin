...
export default function CommissionTemplatesTab({ className = "" }) {
	...
	const handleCreateFromJson = async () => {
		try {
			...
			const response = await api.post("/commission-templates", cleanedData);
	...
	const handleFileUpload = (event) => {
		...
		reader.onload = (e) => {
			try {
				...
				setJsonError(
					`Invalid JSON file format: ${error.message || "Unknown error"}`
				);
			}
		};
	...
	const handleCreateTemplate = async () => {
		try {
			...
			const response = await api.post("/commission-templates", [formData]);
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
					<div className="bg-gray-800 rounded-lg p-6 w-full max-w-md">
						...
						<p className="text-gray-300 mb-6">
							Are you sure you want to delete the commission template
							for "{templateToDelete?.currency}"? This action cannot be
							undone.
						</p>
						<div className="flex justify-end space-x-2">
							<button
...
function CommissionTemplateForm({ formData, setFormData, onSubmit, submitText }) {
