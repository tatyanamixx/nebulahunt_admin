...
interface PackageTemplate {
	...
	description?: string;
	amount: number;
	resource: 'stardust' | 'darkMatter' | 'stars';
	price: number;
	currency: 'tgStars' | 'tonToken' | 'stars' | 'stardust' | 'darkMatter';
	status: boolean;
	icon?: string;
	sortOrder?: number;
	labelKey?: string;
...
interface PackageTemplatesTabProps {
	className?: string;
}

export default function PackageTemplatesTab({
	className = '',
}: PackageTemplatesTabProps) {
	...
	const handleCreateTemplate = async () => {
		try {
			...
			const response = await api.post('/package-templates', [
				cleanedData,
			]);
			...
			showMessage(
				error.response?.data?.message ||
					'Failed to create package template',
				'error'
			);
		}
	};
	...
	const handleUpdateTemplate = async () => {
		try {
			...
			const response = await api.put(
				`/package-templates/${encodeURIComponent(
			...
			showMessage(
				error.response?.data?.message ||
					'Failed to update package template',
				'error'
			);
		}
	};
	...
	const confirmDelete = async () => {
		...
		try {
			...
			showMessage(
				error.response?.data?.message ||
					'Failed to delete package template',
				'error'
			);
		}
	};
	...
	const handleToggleStatus = async (slug: string) => {
		try {
			await api.put(
				`/package-templates/${encodeURIComponent(slug)}/toggle`
			);
			showMessage(
				'Package template status toggled successfully',
				'success'
			);
			fetchTemplates();
		} catch (error: any) {
	...
	const openEditModal = (template: PackageTemplate) => {
	...
	const downloadTemplate = (template: PackageTemplate) => {
...
interface PackageTemplateFormProps {
	formData: Partial<PackageTemplate>;
	setFormData: (data: Partial<PackageTemplate>) => void;
	onSubmit: () => void;
	submitText: string;
}

function PackageTemplateForm({
	formData,
	setFormData,
	onSubmit,
	submitText,
			...
			<div className='grid grid-cols-1 gap-4'>
				<div>
					...
					<input
						...
						onChange={(e) =>
							updateField('labelKey', e.target.value)
						}
						className='w-full p-2 bg-gray-700 border border-gray-600 rounded-md text-white'
			...
			<div className='flex items-center space-x-4'>
				...
				<label className='flex items-center'>
					<input
						...
						onChange={(e) =>
							updateField('isPromoted', e.target.checked)
						}
