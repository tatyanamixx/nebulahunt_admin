...
export default function EventTemplatesTab({
	...
	const handleUpdateTemplate = async () => {
		try {
			...
			showMessage(
				error.response?.data?.message ||
					'Failed to update event template',
				'error'
			);
		}
	};
	...
	const confirmDelete = async () => {
		...
		try {
			...
			showMessage(
				error.response?.data?.message ||
					'Failed to delete event template',
				'error'
			);
		}
	};
	...
	const handleToggleStatus = async (slug: string) => {
		try {
			...
			showMessage(
				'Event template status toggled successfully',
				'success'
			);
			fetchTemplates();
		} catch (error: any) {
	...
	const openEditModal = (template: EventTemplate) => {
	...
	const downloadTemplate = (template: EventTemplate) => {
...
interface EventTemplateFormProps {
	formData: Partial<EventTemplate>;
	setFormData: (data: Partial<EventTemplate>) => void;
	onSubmit: () => void;
	submitText: string;
}
...
function EventTemplateForm({
	...
	const updateJsonField = (field: string, value: string) => {
		try {
				...
				<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
					<div>
						...
						<textarea
							...
							onChange={(e) =>
								updateDescription('en', e.target.value)
							}
					...
					<div>
						...
						<textarea
							...
							onChange={(e) =>
								updateDescription('ru', e.target.value)
							}
							className='w-full h-20 p-3 bg-gray-700 border border-gray-600 rounded-md text-white text-sm'
			...
			<div className='space-y-4'>
				...
				<div>
					...
					<textarea
						...
						onChange={(e) =>
							updateJsonField('effect', e.target.value)
						}
						className='w-full h-32 p-3 bg-gray-700 border border-gray-600 rounded-md text-white font-mono text-sm'
