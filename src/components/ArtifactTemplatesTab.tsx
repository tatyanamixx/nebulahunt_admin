...
import { artifactTemplateApi } from '../lib/api';
...
interface ArtifactTemplate {
	...
	limitedDurationType?: 'HOUR' | 'DAY' | 'WEEK' | 'MONTH' | 'YEAR';
	limitedDurationValue?: number;
	baseChance?: number;
	createdAt?: string;
	updatedAt?: string;
}

interface ArtifactTemplatesTabProps {
	className?: string;
}

export default function ArtifactTemplatesTab({
	className = '',
}: ArtifactTemplatesTabProps) {
	...
	const [sortBy, setSortBy] = useState<'baseChance' | 'name' | 'rarity'>(
		'baseChance'
	);
	const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('asc');

	const [formData, setFormData] = useState<Partial<ArtifactTemplate>>({
		slug: '',
		name: '',
		description: { en: '', ru: '' },
		rarity: 'COMMON',
	...
	const sortTemplates = (templates: ArtifactTemplate[]) => {
	...
	const handleSort = (field: 'baseChance' | 'name' | 'rarity') => {
		if (sortBy === field) {
	...
	const handleDeleteTemplate = async (template: ArtifactTemplate) => {
	...
	const openEditModal = (template: ArtifactTemplate) => {
	...
	const downloadTemplate = (template: ArtifactTemplate) => {
...
interface ArtifactTemplateFormProps {
	formData: Partial<ArtifactTemplate>;
	setFormData: (data: Partial<ArtifactTemplate>) => void;
	onSubmit: () => void;
	submitText: string;
}

function ArtifactTemplateForm({
	formData,
	setFormData,
	onSubmit,
	submitText,
}: ArtifactTemplateFormProps) {
	const updateField = (field: string, value: any) => {
		setFormData({ ...formData, [field]: value });
	};

	const updateDescription = (lang: 'en' | 'ru', value: string) => {
